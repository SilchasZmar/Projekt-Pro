...
public class PersonServiceImpl implements PersonService {
    ...
    /**
     * .stream mi umožní používat další funkce
     * .map všechny entity převede do DTO
     * .collect vráti z5 seznam API
     * @return
     */
    @Override
    ...
    @Override
    public List<PersonStatistic> getPersonStatistics() {

        List<PersonStatistic> result = new ArrayList<>();
        List<Object[]> personStatistic = personRepository.getPersonStatistics();
        BigDecimal totalRevenue = BigDecimal.ZERO;

        for (Object[] statistic : personStatistic) {
            PersonStatistic person = new PersonStatistic();
            person.setPersonId((Long) statistic[0]);
            person.setPersonName((String) statistic[1]);
            person.setRevenue((BigDecimal) statistic[2]);
            result.add(person);
        }
